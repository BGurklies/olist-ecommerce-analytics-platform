...
var defs = new[] {
    ...
    new { Name = "Unique Customers",
          Dax = "DISTINCTCOUNT('mart dim_customer'[customer_unique_id])",
          Folder = "Customer"         },
    new { Name = "Review Submission Rate",
          Dax = "DIVIDE(\n    CALCULATE(\n        DISTINCTCOUNT('mart fact_sales'[order_id]),\n        NOT ISBLANK('mart fact_sales'[review_score])\n    ),\n    [Total Orders]\n)",
          Folder = "Customer"         },
    // ── Payments ─────────────────────────────────────────────────────────────
    new { Name = "Total Payment Value",
    ...
    new { Name = "Axis Max Payment Type",
          Dax = "CALCULATE(MAXX(VALUES('mart dim_payment_type'[payment_type_name]), [Total Payment Value]),     ALLSELECTED('mart dim_payment_type'[payment_type_name])) * 1.15",
          Folder = "Display\\AxisMax" },
